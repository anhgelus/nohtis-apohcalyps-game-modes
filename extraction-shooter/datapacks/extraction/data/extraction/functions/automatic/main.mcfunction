execute at @e[tag=extract,type=minecraft:area_effect_cloud] if block ~ ~ ~ minecraft:oak_trapdoor[open=true] if score @p extract matches 0 run function extraction:private/extract
...
execute if score game.time config = game.tick config run function extraction:private/end
execute if score border.tick config = game.time config run function extraction:private/border
